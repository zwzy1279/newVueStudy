...
export default {
  ...
  computed: {
    ...
    //将来提交订单最终选中地址
    userDefaultAddress() {
      //我们上面点击的是谁，在最终选中地址处就显示谁
      //find:查找数组当中符合条件的元素返回，值为最终结果   条件：isDefault==1
  ...
  methods: {
    ...
    changeDefault(address, addressInfo) {
      //排他思想，先让所有的isDefault都为0，再让点击的为1
      addressInfo.forEach((item) => (item.isDefault = 0));
      address.isDefault = 1;
    },
    //提交订单
    async submitOrder() {
      //以前我们是通过派发action，通知仓库去向服务器发送请求，返回数据存至仓库中，再拿来组件中动态展示数据的
      //现在我们不用到仓库vuex，直接在组件向服务器发送请求，返回数据存至该组件中，直接展示数据
      //console.log(this.$API);

      //提交订单的请求需要传交易编号和用户信息和商品信息给服务器
      //交易编码
      ...
      //在向服务器发送请求的时候将这两个参数传给服务器
      //this.$API.reqSubmitOrder(tradeNo,data);
      let result=await this.$API.reqSubmitOrder(tradeNo,data);
      console.log(result);
      //服务器返回成功的话，将服务器返回的数据存至组件中，跳转到支付Pay的路由组件（路由跳转的时候顺便将服务器返回存至Trade组件的数据通过参数传给Pay组件）
      if(result.code==200){
        //将服务器返回的数据存至组件中
        this.orderId=result.data;
        //跳转到支付Pay的路由组件
        //路由跳转的时候顺便将服务器返回存至Trade组件的数据通过参数传给Pay组件
